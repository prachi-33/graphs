...
// Assuming these types are defined in your lib file
// e.g.,
...
const ALGORITHM_CODE: Record<string, { id: string; code: string }[]> = {
  dijkstra: [
    ...
    <div className="flex flex-col gap-2 h-full">
      <div className="flex gap-2 flex-1 min-h-0">
        ...
        <div className="flex flex-col border border-gray-300 rounded bg-gray-50 basis-[60%] min-w-0">
          ...
          {/* This div handles the vertical scrolling */}
          <div className="flex-1 overflow-y-auto p-2 font-mono text-xs">
            {codeLines.map((line, idx) => (
              ...
              >
                {/* MODIFICATION: Added 'whitespace-pre-wrap'
                  This preserves indentation (like <pre>) but also 
                  wraps long lines of code, preventing horizontal overflow.
                */}
      ...
      {/* Time Complexity Bar */}
      <div className="border border-gray-300 rounded bg-lime-200 px-3 py-2 text-xs text-gray-800">
        <span className="font-semibold text-gray-900">Time Complexity:</span>{' '}
        {algorithm === 'dijkstra'
          ? 'O((V + E) log V) with a priority queue.'
          : 'O(V * E) - Detects negative cycles in O(V * E) as well.'}
      </div>
    </div>
  );
};